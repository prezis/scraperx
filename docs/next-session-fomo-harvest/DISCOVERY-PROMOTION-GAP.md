...
| ⚠ store freshness | `max(discovered_users.last_seen_at)` = **2026-09-22 11:00 BST** — no new sighting for ~56 h at this read. Measured: since then the queue completed only `swaps` 23 · `lifetime_pnl` 7 · `trades` 1 — **zero `followers`/`following` pages**, and the discovery writer only runs on those. Last page of any kind 09-23 23:44 BST. WHY (measured 19:12): the author (`fomo_harvest_author.py`) has NO crontab line and its last `author:*` task was authored 2026-09-18 10:06; the extension polls get `queue_empty`. Dry-run: 34 followers + 160 following seeds ready. Operator 19:1x: he did NOT pause it. ⚠ Do NOT just cron it: since #552 the 47k union rows sit in `wallets` as PROFILE_UNTESTED and author lane (b) (`:617-656`, no min-trades/batch/403 gate) now counts **47,048** `/trades` candidates. (Corrected 19:24: the server caps issue at 180 pages/account/day, 132 after a 403, trades 400/day — `fomo_harvest_store.py:998-1060` — so this would be a months-long DAILY /trades sweep eating the budget, not a ban-shaped burst.) Letter to bot-gate `20260924-191304` asks them to ground this before any `--apply` |
| ▶ 19:15 BST restart | operator order (goal b, „tak, dla wszystkich”): `--only-kinds followers,following --max-new 446 --apply` → **445 queued** (followers 34, following 411), `/trades` 0 | `fomo_harvest.db::tasks` |
| ⚠ `in_wallets_at_discovery` | the same run filled 26,329 NULLs → now 1: 47,184 · 0: 2,711. For rows discovered 09-18..09-22 the „1” means „in `wallets` via the #552 union at fill time 09-24”, NOT „at discovery”. The 17.09 row above (2,677 / 44 %) is NOT comparable with today's split | `fomo_harvest_author.py:785-792` | `fomo_harvest.db::tasks`, `raw_pages` |
