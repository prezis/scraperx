...
| ⚠ store freshness | `max(discovered_users.last_seen_at)` = **2026-09-22 11:00 BST** — no new sighting for ~56 h at this read. Measured: since then the queue completed only `swaps` 23 · `lifetime_pnl` 7 · `trades` 1 — **zero `followers`/`following` pages**, and the discovery writer only runs on those. Last page of any kind 09-23 23:44 BST. WHY (measured 19:12): the author (`fomo_harvest_author.py`) has NO crontab line and its last `author:*` task was authored 2026-09-18 10:06; the extension polls get `queue_empty`. Dry-run: 34 followers + 160 following seeds ready. Operator 19:1x: he did NOT pause it. ⚠ Do NOT just cron it: since #552 the 47k union rows sit in `wallets` as PROFILE_UNTESTED and author lane (b) (`:617-656`, no min-trades/batch/403 gate) now counts **47,048** `/trades` candidates. Letter to bot-gate `20260924-191304` asks them to ground this before any `--apply` |
